...
class PointEncoder(nn.Module):
    ...
    def execute(self, pos, knn_idx, knn_idx_l):
        """
            pos: (B, N, 3)
            knn_idx: (B, N, K)
        """
...
class Network(nn.Module):
    ...
    def execute(self, pos):
        ...
        nn_feat = jt.concat([nn_pc, nn_feat], dim=-1)
        ...
        feat = feat.max(dim=3, keepdims=False)

        ### Weighted Output
        feat = self.conv_1(feat.transpose(2, 1))
        weights = 0.01 + jt.sigmoid(self.conv_w(feat))
        normal = self.mlp_n(self.conv_2(feat * weights).max(dim=2, keepdims=False))

        normal = jt.normalize(normal, p=2, dim=-1)
    ...
    def get_loss(self, q_target, q_pred, pred_weights=None, normal_loss_type='sin', pcl_in=None, trans=None):
        ...
        """
        def cos_angle(v1, v2):
            return nn.bmm(v1.unsqueeze(1), v2.unsqueeze(2)).view(-1) / jt.clamp(v1.norm(2, 1) * v2.norm(2, 1), min=0.000001)

        weight_loss = jt.zeros(1, dtype=q_pred.dtype)

        ### query point normal
        o_pred = q_pred
        o_target = q_target

        o_pred = nn.bmm(o_pred.unsqueeze(1), trans.transpose(2, 1)).squeeze(1)

        if normal_loss_type == 'mse_loss':
            normal_loss = 0.5 * nn.mse_loss(o_pred, o_target)
        elif normal_loss_type == 'ms_euclidean':
            normal_loss = 0.1 * jt.min((o_pred-o_target).pow(2).sum(1), (o_pred+o_target).pow(2).sum(1)).mean()
        elif normal_loss_type == 'ms_oneminuscos':
            cos_ang = cos_angle(o_pred, o_target)
            normal_loss = 1.0 * (1-jt.abs(cos_ang)).pow(2).mean()
        elif normal_loss_type == 'sin':
            normal_loss = 0.1 * jt.norm(jt.cross(o_pred, o_target, dim=-1), p=2, dim=1).mean()
        else:
        ...
        pred_weights = my_squeeze(pred_weights)
        
        if pred_weights is not None:
            thres_d = 0.05 * 0.05
            normal_dis = my_squeeze(nn.bmm(o_target.unsqueeze(1), pcl_in.transpose(2, 1)).pow(2))
            sigma = jt.mean(normal_dis, dim=1) * 0.3 + 1e-5
            threshold_matrix = jt.ones_like(sigma) * thres_d
            sigma = jt.ternary(sigma < thres_d, threshold_matrix, sigma)
            true_weight = jt.exp(-1 * jt.divide(normal_dis, sigma.unsqueeze(-1)))
        ...
        z_vector = my_squeeze(jt.array(np.array([0, 0, 1]).astype(np.float32))).repeat(batch_size, 1)
        z_vector_rot = nn.bmm(z_vector.unsqueeze(1), trans.transpose(2, 1)).squeeze(1)
        z_vector_rot = jt.normalize(z_vector_rot, dim=1)
        z_trans_loss = 0.5 * jt.norm(jt.cross(z_vector_rot, o_target, dim=-1), p=2, dim=1).mean()
