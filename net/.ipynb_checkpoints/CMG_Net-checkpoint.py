...
class Network(nn.Module):
    ...
    def execute(self, pos):
        ...
        nn_feat = jt.cat([nn_pc, nn_feat], dim=-1)
    ...
    def get_loss(self, q_target, q_pred, pred_weights=None, normal_loss_type='sin', pcl_in=None, trans=None):
        ...
        weight_loss = jt.zeros(1, device=q_pred.device, dtype=q_pred.dtype)
        ...
        if pred_weights is not None:
            thres_d = 0.05 * 0.05
            normal_dis = nn.bmm(o_target.unsqueeze(1), pcl_in.transpose(2, 1)).pow(2).squeeze()
            sigma = jt.mean(normal_dis, dim=1) * 0.3 + 1e-5
            threshold_matrix = jt.ones_like(sigma) * thres_d
            sigma = jt.where(sigma < thres_d, threshold_matrix, sigma)
            true_weight = jt.exp(-1 * jt.div(normal_dis, sigma.unsqueeze(-1)))

            weight_loss = (true_weight - pred_weights).pow(2).mean()
        
        regularizer_loss = 0.1 * nn.MSELoss()(trans * trans.permute(0, 2, 1),
                                              init.eye(3, device=trans.device).unsqueeze(0).repeat(
                                              trans.size(0), 1, 1))

        batch_size = trans.shape[0]
        z_vector = jt.array(np.array([0, 0, 1]).astype(np.float32)).squeeze().repeat(batch_size, 1).to(trans.device)
