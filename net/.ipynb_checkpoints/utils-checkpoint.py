...
class GraphConv_H(nn.Module):
    ...
    def get_edge_feature(self, x, pos, knn_idx):
        ...
        knn_dist = jt.sum(knn_pos ** 2, dim=1, keepdim=True)
        knn_r = jt.sqrt(knn_dist.max(dim=3, keepdim=True)[0])
        ...
        if self.neighbor_feature == 2:
            ...
            knn_x = knn_x - x_tiled
            knn_xdist = jt.sum(knn_x ** 2, dim=1, keepdim=True)
            knn_xr = jt.sqrt(knn_xdist.max(dim=3, keepdim=True)[0])
            knn_x = knn_x / knn_xr.expand_as(knn_x)
        
        edge_pos = jt.cat([pos_tiled, knn_pos, knn_x], dim=1)
        return edge_pos
...
class HierarchicalLayer(nn.Module):
    ...
    def execute(self, x, x_last=None, knn_idx=None, pos=None):
        ...
        ori_x = x
        y = self.conv_in(x)
        x_global = jt.max(y, dim=2, keepdim=False)[0]
        if self.with_fc:
        ...
        if self.with_last:
            x = jt.cat([x_global.view(BS, -1, 1).repeat(1, 1, self.output_scale),
                        x_last.view(BS, -1, 1).repeat(1, 1, self.output_scale), x], dim=1)
        else:
            x = jt.cat([x_global.view(BS, -1, 1).repeat(1, 1, self.output_scale), x], dim=1)
